...
const Description = () => {
  ...
  const handleDescViewChange = (view) => {
    setDescView(view);
  };
        ...
        <div className="2xl:border-2 border-slate-200 bg-white 2xl:w-full max-xl:w-[80%] max-md:w-[95%]  2xl:h-screen rounded-md py-6 md:px-4 2xl:px-0">
          <div className="mx-7">
            <div className="flex items-center justify-between">
              <p>G-Doc Review</p>
              <span className="w-7 h-7 bg-red-600 flex items-center justify-center text-white rounded-md cursor-pointer " onClick={cancel_button}>
          ...
          <div className="border-b-1 border-gray-300 text-sm flex px-5 justify-between  mt-10 ">
            <span className={`${descView === 'Overview' && 'border-b-3 border-primary'} px-0.5 cursor-pointer`} onClick={() => handleDescViewChange('Overview')} >Overview</span>
            <span className={`${descView === 'Preview' && 'border-b-3 border-primary'} px-0.5 cursor-pointer`} onClick={() => handleDescViewChange('Preview')} >Preview</span>
            <span className={`${descView === 'History' && 'border-b-3 border-primary'} px-0.5 cursor-pointer`} onClick={() => handleDescViewChange('History')} >History</span>
          </div>
          <Section />
        </div>

      ) : (
        <div className="flex items-center text-xl font-bold">No Description Available</div>
      )}
      </div>
    </section>
  );
